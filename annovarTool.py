    ...
    if os.path.isfile(path):
        ...
        for file in os.listdir(path):
            if file.endswith(".vcf"):
                ...
                os.system(f"perl table_annovar.pl {full_file_path} {db_path} -buildver hg19 -out {destination_path + os.path.basename(file) + '_' + datetime.now().strftime('%Y-%m-%d_%H_%M_%S')} -remove -protocol refGene,cytoBand,exac03,avsnp147,dbnsfp30a -operation g,r,f,f,f -nastring . -vcfinput -polish")
            else:
                print(f"Error: {file} is not a .vcf file")
    else:
